...
public class SwipeListViewTouchListener implements View.OnTouchListener {
    ...
    private List<Boolean> imOpenDontCloseMe = new ArrayList<Boolean>();
    private List<Boolean> imOpenDontCloseMeRight = new ArrayList<Boolean>();
	...
	private boolean startedSwipingRight;
    ...
    public void resetItems() {
        if (swipeListView.getAdapter() != null) {
            ...
            for (int i = opened.size(); i <= count; i++) {
                ...
                checked.add(false);
                imOpenDontCloseMe.add(false);
                imOpenDontCloseMeRight.add(false);
            }
        }
    }
    ...
    private void openAnimate(View view, int position) {
        if (!opened.get(position)) {
            generateRevealAnimate(view, true, false, position, 0);
        }
    }
     ...
     * @param position  Position of list
     */
    private void generateAnimate(final View view, final boolean swap, final boolean swapRight, final int position, final float velocity) {
        ...
        if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_REVEAL) {
            generateRevealAnimate(view, swap, swapRight, position, velocity);
        }
     ...
     * @param position  list position
     */
    private int generateStopX(final boolean swap, final boolean swapRight, final int position) {
        int moveTo = 0;
        if (opened.get(position)) {
            if (!swap) {
                moveTo = openedRight.get(position) ? (int) (viewWidth - rightOffset) : (int) (-viewWidth + leftOffset);
    ...
    private void generateRevealAnimate(final View view, final boolean swap, final boolean swapRight, final int position, float velocity) {
        int moveTo = 0;
        ...
        //adding in velocity for flings
        final float MAX_VELOCITY = 4000;
        if (velocity > MAX_VELOCITY) velocity = MAX_VELOCITY;
        if (velocity < minFlingVelocity) velocity = minFlingVelocity;
        float velocityPct = (velocity - minFlingVelocity) / (float)(MAX_VELOCITY - minFlingVelocity);
        velocityPct = 1 - velocityPct;
        
        long duration = (long) (animationTime * velocityPct);
    ...
    private void resetCell() {
        if (downPosition != ListView.INVALID_POSITION) {
            ...
            //FIXME: frontView.setClickable(opened.get(downPosition));
            //FIXME: frontView.setLongClickable(opened.get(downPosition));
            frontView = null;
            backView = null;
    ...
    void closeOpenedItems() {
        if (opened != null) {
            ...
            for (int i = start; i <= end; i++) {
                if (opened.get(i)) {
                	imOpenDontCloseMe.set(i, false);
                	imOpenDontCloseMeRight.set(i, false);
        ...
        if (opened != null) {
            ...
            for (int i = start; i <= end; i++) {
                if (i == position && opened.get(i)) {
                    setTranslationX(swipeListView.getChildAt(i - start).findViewById(swipeFrontView), 0);
                    swipeListView.resetScrolling();
                    opened.set(position, false);
                    openedRight.set(position, false);
                    imOpenDontCloseMe.set(position, false);
                    imOpenDontCloseMeRight.set(position, false);
                    resetCell();
                }
            }
        }
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (minFlingVelocity <= velocityX && velocityX <= maxFlingVelocity && velocityY * 2 < velocityX) {
                    ...
                    if (swapRight != swipingRight && swipeActionLeft != swipeActionRight) {
                        ...
                        if ((startedSwipingRight && swapRight) || (!startedSwipingRight && !swapRight)) {
                        	swap = true;
                        }
                        else swap = false;
                    }
                } else /*if (Math.abs(deltaX) > viewWidth / 2)*/ {
                	float ox = (deltaX > 0) ? viewWidth - leftOffset : viewWidth - rightOffset;
                	if (Math.abs(deltaX) > ox / 2) {
	                    swap = true;
	                    swapRight = deltaX > 0;
                	}
                }
                
                if (imOpenDontCloseMe.get(downPosition)) {
                	swap = !opened.get(downPosition);
                	swapRight = imOpenDontCloseMeRight.get(downPosition);
                }
                
            	generateAnimate(frontView, swap, swapRight, downPosition, velocityX);
...
//                }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (deltaMode > slop && swipeCurrentAction == SwipeListView.SWIPE_ACTION_NONE && velocityY < velocityX) {
                	...
                	if (!swiping) {
                		swiping = true;
                		startedSwipingRight = swipingRight;
                	}
                ...
                if (swiping && downPosition != ListView.INVALID_POSITION) {
                    if (opened.get(downPosition)) {
                        ...
                        //deltaX += ViewHelper.getX(frontView);
                    }
                    move(deltaX);
                    return true;
                }
    ...
    public void move(float deltaX) {
        ...
        //Log.d("SwipeListView", String.format("**** swipingRight=%s deltaX=%s", String.valueOf(swipingRight), String.valueOf(deltaX)));

        int openState = 0;
        if (opened.get(downPosition)) {
        	if (openedRight.get(downPosition)) openState = 2;
        	else openState = 1;
        }

        boolean okToStop = (openState == 2 && deltaX < 0) ||
        		(openState == 1 && deltaX > 0);
        		
        if (okToStop) {
			deltaX = 0;
	        imOpenDontCloseMe.set(downPosition, true);
	        imOpenDontCloseMeRight.set(downPosition, false /*true*/);
		}
		/*else {
	        imOpenDontCloseMe.set(downPosition, false);
	        imOpenDontCloseMeRight.set(downPosition, false);
		}*/
        
        if (swipingRight) {
        	float pos = (viewWidth - rightOffset);
        	if (deltaX > pos) {
        		deltaX = pos;
                imOpenDontCloseMe.set(downPosition, true);
                imOpenDontCloseMeRight.set(downPosition, true);
        	}
        	else {
        		imOpenDontCloseMe.set(downPosition, false);
                imOpenDontCloseMeRight.set(downPosition, false);
        	...
        	float pos = -(viewWidth - leftOffset);
        	if (deltaX < pos) {
        		deltaX = pos;
                imOpenDontCloseMe.set(downPosition, true);
        	}
    ...
    protected void performDismiss(final View dismissView, final int dismissPosition, boolean doPendingDismiss) {
        ...
        closeOpenedItems(dismissPosition);

        /*opened.set(dismissPosition, false);
        openedRight.set(dismissPosition, false);
        imOpenDontCloseMe.set(dismissPosition, false);
        imOpenDontCloseMeRight.set(dismissPosition, false);*/
